...
public class GPUSkinningSamplerEditor : Editor 
{
    ...
    private void OnGUI_Preview(GPUSkinningSampler sampler)
    {
        BeginBox();
        {
            ...
            if (GUILayout.Button("Set Sampling Resources"))
            {

                string dirPath = "Assets/GameResources/GPUSkinning";
                string animationName = sampler.animName;

                sampler.anim = AssetDatabase.LoadAssetAtPath<GPUSkinningAnimation>(dirPath + "/"+ GPUSkinningHelper.GPUSKinningAnim( animationName ));
                sampler.savedMesh = AssetDatabase.LoadAssetAtPath<UnityEngine.Mesh>(dirPath + "/" + GPUSkinningHelper.GPUSKinningMesh(animationName));
                sampler.savedMtrl = AssetDatabase.LoadAssetAtPath<UnityEngine.Material>(dirPath + "/" + GPUSkinningHelper.GPUSKinningMaterial(animationName));
                sampler.texture = AssetDatabase.LoadAssetAtPath<UnityEngine.TextAsset>(dirPath + "/" + GPUSkinningHelper.GPUSKinningMatrixTexture(animationName));
