...
namespace GPUSkingings
{
    ...
    public class GPUSkinningMeshExport
    {
        ...
        public string version = "LAYAMODEL:GPUSkining_06";
        ...
        Vector3 boundsMin = Vector3.one * -1;
        ...
        public void SetMesh(Mesh mesh)
        {
            ...
            GameObject go = new GameObject("MeshBound");
            SkinnedMeshRenderer meshRender = go.AddComponent<SkinnedMeshRenderer>();
            meshRender.sharedMesh = mesh;
            Bounds bounds = meshRender.bounds;
            GameObject.DestroyImmediate(go);
            boundsMin.x = bounds.min.x;
            boundsMin.y = bounds.min.z;
            boundsMin.z = bounds.min.y;


            boundsMax.x = bounds.max.x;
            boundsMax.y = bounds.max.z;
            boundsMax.z = bounds.max.y;
            //Debug.Log(bounds);
        }
        ...
        public void Export(string outDir = null)
        {
            ...
            // bound
            w.WriteVector3(boundsMin);
            w.WriteVector3(boundsMax);
