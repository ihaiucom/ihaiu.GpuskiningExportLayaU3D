    ...
    private void CreateTextureMatrix(string dir, GPUSkinningAnimation gpuSkinningAnim)
    {
        ...
        for (int clipIndex = 0; clipIndex < gpuSkinningAnim.clips.Length; ++clipIndex)
        {
            ...
            for (int frameIndex = 0; frameIndex < numFrames; ++frameIndex)
            {
                ...
                if(frame == null)
                {
                    UnityEngine.Debug.LogError(gpuSkinningAnim.name + " " + clip.name + "  frame=" + frameIndex + " 该帧数据是空的" );
                }
        ...
        string savedPath = dir + "/" + GPUSkinningHelper.GPUSKinningMatrixTexture(animName);
        ...
        if (samplingFrameIndex >= totalFrams)
        {
			...
			if(!string.IsNullOrEmpty(savePath))
			{
				...
				else
				{
                    ...
                    string savedAnimPath = dir + "/" + GPUSkinningHelper.GPUSKinningAnim(animName);
                    ...
                    if (samplingClipIndex == 0)
                    {
                        ...
                        string savedMeshPath = dir + "/" + GPUSkinningHelper.GPUSKinningMesh(animName);
                        AssetDatabase.CreateAsset(newMesh, savedMeshPath);
